...
namespace Hydrology.CControls
{
    ...
    class CDataGridViewVoltage : CExDataGridView
    {
        ...
        public static readonly string CN_Visi1min = "1分钟能见度";
        public static readonly string CN_Visi10min = "10分钟能见度";
            ...
            : base()
        {
            ...
            { 
                CS_StationID,CS_StationName,CS_TimeCollected, CN_Visi1min,CS_DataState,CN_Visi10min, CS_TimeReceived, CS_ChannelType, CS_MsgType 
            };
        ...
        public void InitDataSource(IPWDProxy proxy)
        {
            m_proxyPwd = proxy;
        }

        // 设置显示的电压记录
        public void SetVoltage(List<PWDData> listPwd)
        {
            ...
            {
                // 只读模式
                for (int i = 0; i < listPwd.Count; ++i)
                {
                    CEntityStation station = CDBDataMgr.Instance.GetStationById(listPwd[i].StationID);
                    ...
                    string[] newRow;
                    if (listPwd[i].BState == 0)
                    {
                        // 不正常
                        state = CExDataGridView.EDataState.EError;//红色显示 
                        state_1 = CS_DataState_AbNormal;
                    }   
                    if (listPwd[i].Visi1min < 0 || listPwd[i].Visi10min < 0)
                    {
                        newRow = new string[]
                    {
                        strStationId,
                        strStationName, /*站名*/
                        listPwd[i].TimeCollect.ToString(CS_TimeFormat), /*采集时间*/
                        "--", 
                        state_1,
                        "--",
                        listPwd[i].TimeRecieved.ToString(CS_TimeFormat), /*接收时间*/
                        CEnumHelper.ChannelTypeToUIStr(listPwd[i].ChannelType), /*通讯方式*/
                        CEnumHelper.MessageTypeToUIStr(listPwd[i].MessageType) /*报文类型*/
                    };
                    }
                    else
                    {
                        newRow = new string[]
                    {
                        ...
                        strStationName, /*站名*/
                        listPwd[i].TimeCollect.ToString(CS_TimeFormat), /*采集时间*/
                        listPwd[i].Visi1min.ToString(), /*电压值*/
                        state_1,
                        listPwd[i].Visi10min.ToString(), /*电压值*/
                        listPwd[i].TimeRecieved.ToString(CS_TimeFormat), /*接收时间*/
                        CEnumHelper.ChannelTypeToUIStr(listPwd[i].ChannelType), /*通讯方式*/
                        CEnumHelper.MessageTypeToUIStr(listPwd[i].MessageType) /*报文类型*/
                    };
        ...
        public bool SetFilter(string strStationId, DateTime timeStart, DateTime timeEnd, bool TimeSelect)
        {
            ...
            m_proxyPwd.SetFilter(strStationId, timeStart, timeEnd, TimeSelect);
            if (-1 == m_proxyPwd.GetPageCount())
            {
            ...
            else
            {
                ...
                base.TotalPageCount = m_proxyPwd.GetPageCount();
                base.TotalRowCount = m_proxyPwd.GetRowCount();
                SetVoltage(m_proxyPwd.GetPageData(1, false)) ;
                return true;
            }
        ...
        public void SetEditable(bool bEditable)
        {
            ...
            if (m_bIsEditable)
            {
                ...
                this.Header = new string[] 
                { 
                    CS_Delete,CS_StationID,CS_StationName, CS_TimeCollected,CN_Visi1min,CS_DataState,CN_Visi10min, CS_TimeReceived, CS_ChannelType, CS_MsgType
                };
        ...
        protected override void OnMenuPreviousPage(object sender, EventArgs e)
        {
            ...
            if (this.IsModifiedUnSaved())
            {
                ...
                else if (DialogResult.Yes == result)
                {
                    ...
                    // 保存成功，换页
                    //SetVoltage(m_proxyPwd.GetPageData(base.m_iCurrentPage - 1));
        ...
        protected override void OnMenuLastPage(object sender, EventArgs e)
        {
            if (this.IsModifiedUnSaved())
            {
                ...
                else if (DialogResult.No == result)
                {
                    ...
                    base.ClearAllState();
                    base.OnMenuLastPage(sender, e);
                    SetVoltage(m_proxyPwd.GetPageData(base.m_iCurrentPage,false));
                    this.UpdateDataToUI();
                }
            }
            else
