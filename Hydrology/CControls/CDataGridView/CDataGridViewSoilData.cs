...
namespace Hydrology.CControls
{
    ...
    class CDataGridViewSoilData : CExDataGridView
    {
        ...
        public static readonly string CS_Depth = "深度";
        public static readonly string CS_NTU = "浊度";
        public static readonly string CS_ppm = "泥度";
        public static readonly string CS_Temperature = "温度";
        public static readonly string CS_Conductivity = "导电率";
        public static readonly string CS_Salinity = "盐度";
        public static readonly string CS_Batt = "电压";
        ...
        public void InitDataSource(IOBSProxy proxy)
        {
            m_proxyObs = proxy;
        }
        ...
        // 设置显示的雨量记录
        public void SetSoilData(List<CEntityOBS> listObsData)
        {
            ...
            {
                // 只读模式
                for (int i = 0; i < listObsData.Count; ++i)
                {
                    ...
                    CEntityStation station = CDBDataMgr.Instance.GetStationById(listObsData[i].StationID);
                    if (null != station)
                    {
                    ...
                    string[] newRow = new string[]
                    {
                        strStationID,
                        strStationName,
                        listObsData[i].TimeCollect.ToString(CS_TimeFormat), /*采集时间*/

                        listObsData[i].Depth.ToString(),
                        listObsData[i].NTU.ToString(),
                        listObsData[i].ppm.ToString(),
                        listObsData[i].Temperature.ToString(),
                        listObsData[i].Conductivity.ToString(),
                        listObsData[i].Salinity.ToString(),
                        listObsData[i].Batt.ToString(),
                        listObsData[i].TimeRecieved.ToString()
        ...
        public bool SetFilter(string iStationId, DateTime timeStart, DateTime timeEnd,bool TimeSelect)
        {
            ...
            m_proxyObs.SetFilter(m_strStaionId, timeStart, timeEnd, TimeSelect);
            if (-1 == m_proxyObs.GetPageCount())
            {
            ...
            else
            {
                ...
                base.TotalPageCount = m_proxyObs.GetPageCount();
                base.TotalRowCount = m_proxyObs.GetRowCount();
                SetSoilData(m_proxyObs.GetPageData(1, false));
                return true;
            }
        }
