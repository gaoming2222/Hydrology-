...
#pragma warning restore CS0168 // 声明了变量“e8”，但从未使用过
            ...
            else if (str.Contains("@@ OBS"))
            {
                ...
                if (report.obsDatas.Count == 0)
                {
                    string rawStr = e.RawData;
                    //report.Datas.Add(WrongParser(rawStr));
                }
                //处理obs数据
                foreach (var item in report.obsDatas)
                {
                    stationDatas.obsDatas.Add(new CSingleStationObsData()
                    {
                        dataTime = item.Time,
                        Depth = item.Depth,
                        Ntu = item.Ntu,
                ...
                CDBDataMgr.Instance.EHRecvOBSDatas(null, stationDatas);
            }
            ///处理En2b数据
            else if (str.Contains("@@  EN2B"))
            {
                var stationDatas = new CEventRecvStationDatasArgs()
                {
                    StrStationID = report.Stationid,
                ...
                if (report.fsfxDatas.Count == 0)
                {
                    string rawStr = e.RawData;
                    //report.Datas.Add(WrongParser(rawStr));
                }
                //处理obs数据
                CSingleStationEn2BData pwdData = new CSingleStationEn2BData();
                if (report.pwdDatas != null && report.pwdDatas.Count > 0)
                {
                    foreach (var item in report.pwdDatas)
                    {
                        pwdData.dataTime = item.Time;
                        pwdData.Visi10min = item.Visi10min;
                        pwdData.Visi1min = item.Visi1min;
                    }
                }

                CSingleStationEn2BData fsfxData = new CSingleStationEn2BData();
                if (report.fsfxDatas != null || report.fsfxDatas.Count > 0)
                {
                    foreach (var item in report.fsfxDatas)
                    {
                        //fsfxData.
                        fsfxData.dataTime = item.Time;
                        fsfxData.water = item.Water;
                        fsfxData.shfs = item.shfs;
                        fsfxData.yxszdshfx = item.yxszdshfx;
                        fsfxData.yxszdshfs = item.yxszdshfs;
                        fsfxData.maxTime = item.maxTime;
                        fsfxData.avg2fx = item.avg2fx;
                        fsfxData.avg2fs = item.avg2fs;
                        fsfxData.avg10fx = item.avg10fx;
                        fsfxData.avg10fs = item.avg10fs;
                        fsfxData.max10fx = item.max10fx;
                        fsfxData.max10fs = item.max10fs;
                        fsfxData.max10tm = item.max10tm;
                        fsfxData.Voltage = item.Voltge;
                    }
                }
                stationDatas.en2bDatas.Add(new CSingleStationEn2BData()
                {
                    dataTime = fsfxData.dataTime,
                    water = fsfxData.water,
                    shfs = fsfxData.shfs,
                    yxszdshfx = fsfxData.yxszdshfx,
                    yxszdshfs = fsfxData.yxszdshfs,
                    maxTime = fsfxData.maxTime,
                    avg2fx = fsfxData.avg2fx,
                    avg2fs = fsfxData.avg2fs,
                    avg10fx = fsfxData.avg10fx,
                    avg10fs = fsfxData.avg10fs,
                    max10fx = fsfxData.max10fx,
                    max10fs = fsfxData.max10fs,
                    max10tm = fsfxData.max10tm,
                    Visi1min = pwdData.Visi1min,
                    Visi10min = pwdData.Visi10min,
                    Voltage = fsfxData.Voltage
                }) ;

                CDBDataMgr.Instance.EHRecvEn2BDatas(null, stationDatas);
            }
