...
namespace Hydrology.DataMgr
{
    class CDBDataMgr
    {
        ...
        private IFSFXProxy m_proxyFsfx;
        private IOBSProxy m_proxyObs;
        private IPWDProxy m_proxyPwd;
        ...
        public bool Init()
        {
            ...
            m_proxyFsfx = new CSQLFsfx();
            m_proxyObs = new CSQLObs();
            m_proxyPwd = new CSQLPwd();
...
#pragma warning restore CS0168 // 声明了变量“e”，但从未使用过
        ...
        public IFSFXProxy GetFxfxProxy()
        {
            return m_proxyFsfx;
        }
        public IOBSProxy GetObsProxy()
        {
            return m_proxyObs;
        }
        public IPWDProxy GetPwdProxy()
        {
            return m_proxyPwd;
        }
...
#pragma warning restore CS0168 // 声明了变量“exp”，但从未使用过
        ...
        /// <param name="args"></param>
        public void EHRecvOBSDatas(object sender, CEventRecvStationDatasArgs args)
        { 
            NewTask(() => { DealObsDatas(args); });
        }

        public void EHRecvEn2BDatas(object sender, CEventRecvStationDatasArgs args)
        {
            NewTask(() => { DealEn2bDatas(args); });
        }
...
#pragma warning restore CS0168 // 声明了变量“e”，但从未使用过
        ...
        private void DealObsDatas(CEventRecvStationDatasArgs args)
        {
            try
            {
                //1.判定是否有数据，如果没有数据则返回
                int tmpDataCount = args.obsDatas.Count;
                if (tmpDataCount <= 0)
                {
                    // 数据为空
                    CSystemInfoMgr.Instance.AddInfo("收到空的数据记录项目");
                ...
                #region obs数据
                List<CEntityOBS> obsList = new List<CEntityOBS>();
                foreach (CSingleStationObsData data in args.obsDatas)
                {
                    CEntityOBS obsData = new CEntityOBS();
                    obsData.StationID = args.StrStationID;
                    obsData.TimeCollect = data.dataTime;
                    obsData.Depth = data.Depth;
                    obsData.NTU = data.Ntu;
                    obsData.ppm = data.Mud;
                    obsData.Temperature = data.Tmp;
                    obsData.Conductivity = data.Cndcty;
                    obsData.Salinity = data.Salinity;
                    obsData.Batt = data.Voltage;
                    obsData.TimeRecieved = DateTime.Now;
                    obsData.MessageType = args.EMessageType;
                    obsData.ChannelType = args.EChannelType;
                    obsList.Add(obsData);
                }
                if(obsList != null && obsList.Count > 0)
                {
                    m_proxyObs.AddNewRows(obsList);
                }
               #endregion
               ...
               #region 实时表数据
                ...
                realtime.StrStationName = station.StationName;
                realtime.StrStationID = station.StationID;
                realtime.TimeDeviceGained = args.obsDatas[tmpDataCount - 1].dataTime;
                realtime.EIStationType = station.StationType;
                realtime.Depth = args.obsDatas[tmpDataCount - 1].Depth;
                realtime.Ntu = args.obsDatas[tmpDataCount - 1].Ntu;
                realtime.Tmp = args.obsDatas[tmpDataCount - 1].Tmp;
                realtime.Cndcty = args.obsDatas[tmpDataCount - 1].Cndcty;
                realtime.Salinity = args.obsDatas[tmpDataCount - 1].Salinity;
                realtime.Dvoltage = args.obsDatas[tmpDataCount - 1].Voltage;
        ...
        private void DealEn2bDatas(CEventRecvStationDatasArgs args)
        {
            try
            {
                //1.判定是否有数据，如果没有数据则返回
                int tmpDataCount = args.en2bDatas.Count;
                if (tmpDataCount <= 0)
                {
                    // 数据为空
                    CSystemInfoMgr.Instance.AddInfo("收到空的数据记录项目");
                ...
                #region en2b数据
                List<FSFXData> fsfxList = new List<FSFXData>();
                List<PWDData> pwdList  = new List<PWDData>();
                List<CEntityWater> waterList = new List<CEntityWater>();
                foreach (CSingleStationEn2BData data in args.en2bDatas)
                {
                    FSFXData fsfxData = new FSFXData();
                    PWDData pwdData = new PWDData();
                    CEntityWater water = new CEntityWater();

                    //风速风向数据
                    fsfxData.StationID = args.StrStationID;
                    fsfxData.TimeCollect = data.dataTime;
                    fsfxData.shfx = data.shfx;
                    fsfxData.shfs = data.shfs;
                    fsfxData.yxszdshfx = data.yxszdshfx;
                    fsfxData.yxszdshfs = data.yxszdshfs;
                    fsfxData.maxTime = data.maxTime;
                    fsfxData.avg2fx = data.avg2fx;
                    fsfxData.avg2fs = data.avg2fs;
                    fsfxData.avg10fx = data.avg10fx;
                    fsfxData.avg10fs = data.avg10fs;
                    fsfxData.max10fx = data.max10fx;
                    fsfxData.max10fs = data.max10fs;
                    fsfxData.max10tm = data.max10tm;
                    fsfxData.TimeRecieved = DateTime.Now;
                    fsfxData.MessageType = args.EMessageType;
                    fsfxData.ChannelType = args.EChannelType;

                    //能见度数据
                    pwdData.StationID = args.StrStationID;
                    pwdData.TimeCollect = data.dataTime;
                    pwdData.Visi1min = data.Visi1min;
                    pwdData.Visi10min = data.Visi10min;
                    pwdData.TimeRecieved = DateTime.Now;
                    pwdData.MessageType = args.EMessageType;
                    pwdData.ChannelType = args.EChannelType;

                    //水位数据
                    water.StationID = station.StationID;
                    water.TimeCollect = data.dataTime;
                    water.TimeRecieved = DateTime.Now;
                    if (data.water.HasValue)
                    {
                        water.WaterStage = data.water.Value;
                    }
                    water.WaterFlow = null;    //此处需要计算的
                    water.ChannelType = args.EChannelType;
                    water.MessageType = args.EMessageType;
                    ...
                    waterList.Add(water);
                    fsfxList.Add(fsfxData);
                    pwdList.Add(pwdData);
                }
                if (fsfxList != null && fsfxList.Count > 0)
                {
                    m_proxyFsfx.AddNewRows(fsfxList);
                }
                if(pwdList != null && pwdList.Count > 0)
                {
                    m_proxyPwd.AddNewRows(pwdList);
                }
                if(waterList != null && waterList.Count > 0)
                {
                    m_proxyWater.AddNewRows(waterList);
                }
                #endregion
                ...
                realtime.TimeDeviceGained = args.RecvDataTime;
                realtime.EIStationType = station.StationType;
                realtime.SHFX = args.en2bDatas[tmpDataCount - 1].shfx;
                realtime.SHFS = args.en2bDatas[tmpDataCount - 1].shfs;
                realtime.avg10fx = args.en2bDatas[tmpDataCount - 1].avg10fx;
                realtime.avg10fs = args.en2bDatas[tmpDataCount - 1].avg10fs;
                realtime.Visi1min = args.en2bDatas[tmpDataCount - 1].Visi1min;
                realtime.Visi10min = args.en2bDatas[tmpDataCount - 1].Visi10min;
                realtime.Dvoltage = args.en2bDatas[tmpDataCount - 1].Voltage;
                // 发消息，通知界面更新
                if (RecvedRTD != null)
                {
                    Task.Factory.StartNew(() => { RecvedRTD.Invoke(this, new CEventSingleArgs<CEntityRealTime>(realtime)); });
